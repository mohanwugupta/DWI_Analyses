...
addpath(genpath('/u/CHANGE/THIS/PATH/packages/ABTIN-master')); #CHANGE PATH
fid = fopen('abtin_input.txt');
abtin_file = textscan(fid, '%s', 'delimiter', '\n');
cell_file = [abtin_file{1:1}];
InputFolder=cell_file(1,:);
InputFolder=char(InputFolder);
OutputFolder=cell_file(2,:);
OutputFolder=char(OutputFolder);


%% Read stuff
Files = dir([InputFolder '/*_ICVF*']);
if isempty(Files)
    error('no ICVF.nii was found')
end

for i = 1:length(Files)
    nameInd = strfind(Files(i).name,'_ICVF.nii.gz');
    Name = Files(i).name(1:nameInd-1);
    
    sprintf(['Processing ....' Name])
    % Read ficvf
    fic = load_untouch_nii([InputFolder '/' Name '_ICVF.nii.gz']);
    Fic = fic.img;
    
    % Read fiso
    fiso = load_untouch_nii([InputFolder '/' Name '_ISOVF.nii.gz']);
    Fiso = fiso.img;
